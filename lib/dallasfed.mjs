...
export async function withRetry(label, fn, attempts = 3, log = console.log) {
  ...
  for (let i = 1; i <= attempts; i++) {
    try {
      ...
      // BEA meters by volume per MINUTE (100 MB), and the underlying-detail tables are
      // ~11 MB a call. A few seconds of backoff would just spend the remaining attempts
      // against a door that stays shut until the window rolls over, so wait it out.
      const throttled = /quota|rate limit|too many requests/i.test(err.message);
      const wait = throttled ? 65000 : 3000 * i;
